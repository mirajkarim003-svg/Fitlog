...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    getWorkouts()
      ...
      .catch(() => {
        if (!cancelled) setError("Couldn't load the workout library. Try refreshing.");
      })
      .finally(() => {
    ...
    <AppContext.Provider value={value}>
      ...
      <div className="pointer-events-none fixed bottom-5 right-5 z-[100] flex flex-col gap-2">
        {toasts.map((t) => (
          <div
            key={t.id}
            className="pointer-events-auto rounded-lg border border-line bg-surface2 px-4 py-3 text-sm font-medium text-ink shadow-lg shadow-black/40 animate-toast-in"
          >
            {t.message}
          </div>
        ))}
